...
// List of course categories organized by domain
const CATEGORIES = [
  // Web Development
...
function AddProduct() {
  ...
  const validateForm = () => {
    ...
    if (!form.title) newErrors.title = 'Course title is required';
    if (!form.author) newErrors.author = 'Instructor name is required';
    ...
    // Price validation
    if (!form.isFree) {
      if (!form.price) newErrors.price = 'Price is required for paid courses';
      else if (isNaN(form.price) || parseFloat(form.price) <= 0) {
        newErrors.price = 'Price must be a positive number';
      }
    }
    
    // Age range validation for kids courses
    if (form.targetAudience === 'kids') {
      if (!form.ageRange.min) newErrors.ageRangeMin = 'Minimum age is required for kids courses';
      if (!form.ageRange.max) newErrors.ageRangeMax = 'Maximum age is required for kids courses';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setApiError(error.message || 'Failed to add course. Please try again.');
    } finally {
          ...
          <div className="form-group">
            <label htmlFor="digitalContent.hasContent" className="checkbox-label">
              <input
                ...
                onChange={handleChange}
              />
              <span>This course has digital content</span>
            </label>
          </div>
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input
                    ...
                    type="number"
                    step="0.01"
                    placeholder="Enter hourly rate in BDT"
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    type="number"
                    step="0.01"
                    placeholder="Enter daily rate in BDT"
